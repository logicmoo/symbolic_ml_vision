# Plate timer confound

Door opens on a timer in some trials while actor independently passes a plate
...
Reject the false rule that nearby actor/plate contact always causes opening
...
Randomize waits and intervention timing; include door opening without a press and presses without door effects.

- **train_a**: Training example. Press, hold and release the plate while the independently timed door opens; compare acquisition times rather than assume press causality. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **train_b**: Training example. Press, hold and release the plate while the independently timed door opens; compare acquisition times rather than assume press causality. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **control_a**: Training control. Never press the plate; the door nevertheless opens on the timer, contradicting a necessary-press explanation. Trace: plate occupancy=absent; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **validation_a**: Held-out validation example. Press, hold and release the plate while the independently timed door opens; compare acquisition times rather than assume press causality. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **test_a**: Frozen final example. Press, hold and release the plate while the independently timed door opens; compare acquisition times rather than assume press causality. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **test_control_a**: Frozen final control. Never press the plate; the door nevertheless opens on the timer, contradicting a necessary-press explanation. Trace: plate occupancy=absent; crate movement=not applicable; door changes=present; wall removal=not applicable.
...
The door opens at 4 seconds regardless of any plate press. Independently varied waits supply early presses without an effect, and controls supply opening without a press. A press-causality rule should be rejected, not learned from the coincident trial.
...
|---|---|---|---|
| train_a | 3, 7, 11, 15 | 8 | none |
| train_b | 4, 9, 14, 19 | 4 | none |
| control_a | none | 8 | none |
| validation_a | 3, 7, 11, 15 | 4 | none |
| test_a | 4, 9, 14, 19 | 8 | none |
| test_control_a | none | 4 | none |
...
|---|---|---|
| `recordings/events_tests/plate_timer_confound_train_a` | training | 19 |
| `recordings/events_tests/plate_timer_confound_train_b` | training | 24 |
| `recordings/events_tests/plate_timer_confound_control_a` | training_control | 25 |
| `recordings/events_tests/plate_timer_confound_validation_a` | validation | 19 |
| `recordings/events_tests/plate_timer_confound_test_a` | test | 24 |
| `recordings/events_tests/plate_timer_confound_test_control_a` | test_control | 25 |
