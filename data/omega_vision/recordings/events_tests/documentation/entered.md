...
An object crosses the left viewport boundary.

**Execution status: not run.**

## Local memory required for this test

Earlier viewport coverage, clipped silhouette and emerging track history

## Measured new evidence and decision / episode timing

At `1` observe a strip on the left boundary; `2` and `3` support inward continuity. A conservative entry classification can wait for `2`

## Required caution and fixture insufficiency

Do not read the oracle's off-screen position at `0`. Border first-visibility without motion support remains ambiguous.
...
|---|---|---|
| `recordings/events_tests/entered` | event_baseline | 4 |
