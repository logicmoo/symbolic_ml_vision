...
Push two touching crates with a free space beyond, then with a blocked far end
...
Learn multi-object displacement and the downstream-obstacle condition
...
Independently observe all destinations; do not invent recursive propagation or infinite lookahead support.

- **train_a**: Training example. Push both crates while the far end is clear, encounter the wall, remove it with a click and resume chain movement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **train_b**: Training example. Push both crates while the far end is clear, encounter the wall, remove it with a click and resume chain movement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **control_a**: Training control. Attempt the chain with its far end blocked from the outset; neither crate can displace despite near-side contact. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=absent.
- **validation_a**: Held-out validation example. Push both crates while the far end is clear, encounter the wall, remove it with a click and resume chain movement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **test_a**: Frozen final example. Push both crates while the far end is clear, encounter the wall, remove it with a click and resume chain movement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **test_control_a**: Frozen final control. Attempt the chain with its far end blocked from the outset; neither crate can displace despite near-side contact. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=absent.
...
Two crates move together while the far destination is clear, then both stop at a wall. Clicking the wall allows a subsequent chain displacement. Controls begin with a blocked far end, even though the actor contacts the near crate normally.
...
|---|---|---|---|
| train_a | none | none | 3, 4, 11 |
| train_b | none | none | 4, 5, 13 |
| control_a | none | none | none |
| validation_a | none | none | 3, 4, 11 |
| test_a | none | none | 4, 5, 13 |
...
|---|---|---|
| `recordings/events_tests/push_chain_train_a` | training | 16 |
| `recordings/events_tests/push_chain_train_b` | training | 19 |
| `recordings/events_tests/push_chain_control_a` | training_control | 21 |
| `recordings/events_tests/push_chain_validation_a` | validation | 16 |
| `recordings/events_tests/push_chain_test_a` | test | 19 |
| `recordings/events_tests/push_chain_test_control_a` | test_control | 21 |
