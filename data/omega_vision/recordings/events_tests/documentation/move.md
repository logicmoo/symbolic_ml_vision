...
Movement begins, continues, then stops.

**Execution status: not run.**

## Local memory required for this test

Prior measured stationary/moving state, previous displacement and active motion episode

## Measured new evidence and decision / episode timing

After a measured stationary warm-up: stationary -> moving starts; further displacement continues; zero displacement ends

## Required caution and fixture insufficiency

Current `0` is a single still image. Add a versioned identical warm-up frame before expecting `start(move)`; do not initialize velocity to zero by assumption.
...
|---|---|---|
| `recordings/events_tests/move` | event_baseline | 4 |
