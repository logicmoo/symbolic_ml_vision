...
Motion heading changes from rightward to downward.

**Execution status: not run.**

## Local memory required for this test

Two successive displacements and their times

## Measured new evidence and decision / episode timing

At `2`, compare `(4, 0)` from `0 -> 1` with `(0, 4)` from `1 -> 2`; retain the downward continuation at `3`

## Required caution and fixture insufficiency

Distinguish path heading from rotation of an asymmetric sprite; ignore heading when speed is below tolerance.
...
|---|---|---|
| `recordings/events_tests/turned` | event_baseline | 4 |
