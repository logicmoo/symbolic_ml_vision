...
Occupy/release with a reproducible delay before response
...
Learn an effect window and maintain pending obligations until it expires
...
Short clip ending before the horizon is inconclusive, not a negative; vary sample rate using trustworthy timestamps.

- **train_a**: Training example. Occupy and release the plate twice, observing the completed delayed opening and closure windows after the occupancy edges. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **train_b**: Training example. Occupy and release the plate twice, observing the completed delayed opening and closure windows after the occupancy edges. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **control_a**: Training control. Wait and move nearby without pressing; observe the full no-response horizon rather than an early truncated window. Trace: plate occupancy=absent; crate movement=not applicable; door changes=absent; wall removal=not applicable.
- **validation_a**: Held-out validation example. Occupy and release the plate twice, observing the completed delayed opening and closure windows after the occupancy edges. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **test_a**: Frozen final example. Occupy and release the plate twice, observing the completed delayed opening and closure windows after the occupancy edges. Trace: plate occupancy=present; crate movement=not applicable; door changes=present; wall removal=not applicable.
- **test_control_a**: Frozen final control. Wait and move nearby without pressing; observe the full no-response horizon rather than an early truncated window. Trace: plate occupancy=absent; crate movement=not applicable; door changes=absent; wall removal=not applicable.
- **control_horizon**: Inconclusive control. Occupy the plate, then end 0.5 seconds later, before the 1.5-second response horizon; no negative response verdict is justified. Trace: plate occupancy=present; crate movement=not applicable; door changes=absent; wall removal=not applicable.
...
Every occupancy edge is delivered after 1.5 seconds: transport delay, not debounce. Sampling may reveal the effect only at the next acquired frame. The control_horizon clip ends 0.5 seconds after occupancy and is explicitly inconclusive.
...
|---|---|---|---|
| train_a | 3, 7, 11, 15 | 6, 10, 14, 18 | none |
| train_b | 4, 9, 14, 19 | 6, 11, 16, 21 | none |
| control_a | none | none | none |
| validation_a | 3, 7, 11, 15 | 5, 9, 13, 17 | none |
| test_a | 4, 9, 14, 19 | 7, 12, 17, 22 | none |
| test_control_a | none | none | none |
| control_horizon | 3 | none | none |
...
|---|---|---|
| `recordings/events_tests/plate_delayed_train_a` | training | 19 |
| `recordings/events_tests/plate_delayed_train_b` | training | 24 |
| `recordings/events_tests/plate_delayed_control_a` | training_control | 25 |
| `recordings/events_tests/plate_delayed_validation_a` | validation | 19 |
| `recordings/events_tests/plate_delayed_test_a` | test | 24 |
| `recordings/events_tests/plate_delayed_test_control_a` | test_control | 25 |
| `recordings/events_tests/plate_delayed_control_horizon` | inconclusive_control | 5 |
