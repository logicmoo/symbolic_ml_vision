...
The same observed object returns after a brief gap.

**Execution status: not run.**

## Local memory required for this test

Frame `0`'s appearance/identity, missing status at `1`, last-position uncertainty

## Measured new evidence and decision / episode timing

At `2`, independently match the returning observation to the missing track; continue it at `3`

## Required caution and fixture insufficiency

Same color or reused fixture name is not sufficient when several tracks match. Do not retroactively label the gap as proven occlusion.
...
|---|---|---|
| `recordings/events_tests/reappeared` | event_baseline | 4 |
