...
A platform picks up, transports, then leaves its load. Includes the history required before classification.
...
- Frame 2: `{'predicate': 'start', 'args': [{'predicate': 'carry', 'args': ['carrier', 'cargo']}]}` (decision uses only frames 0 through 2).
- Frame 3: `{'predicate': 'continue', 'args': [{'predicate': 'carry', 'args': ['carrier', 'cargo']}]}` (decision uses only frames 0 through 3).
- Frame 4: `{'predicate': 'end', 'args': [{'predicate': 'carry', 'args': ['carrier', 'cargo']}]}` (decision uses only frames 0 through 4).
...
`recordings/events_tests/carry_confirmed_v2` contains 5 numbered frames from 0.
