...
Blue follows the red leader around a corner, then takes a different path. Includes the history required before classification.
...
- Directed lagged path evidence is required; similar simultaneous velocity is only co-motion.
...
- Frame 2: `{'predicate': 'start', 'args': [{'predicate': 'follow', 'args': ['actor', 'other']}]}` (decision uses only frames 0 through 2).
- Frame 3: `{'predicate': 'continue', 'args': [{'predicate': 'follow', 'args': ['actor', 'other']}]}` (decision uses only frames 0 through 3).
- Frame 4: `{'predicate': 'end', 'args': [{'predicate': 'follow', 'args': ['actor', 'other']}]}` (decision uses only frames 0 through 4).
...
`recordings/events_tests/follow_confirmed_v2` contains 5 numbered frames from 0.
