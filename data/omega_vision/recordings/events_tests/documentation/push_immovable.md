...
Try visibly similar but behaviorally different objects
...
Learn evidence-backed movable/immovable affordances rather than using color as a label
...
Recolor or relocate objects; retain counterexamples and ambiguity instead of inventing hidden mass facts.

- **train_a**: Training example. Approach and repeatedly attempt to push an object with visible space beyond it; it stays fixed while the actor can retreat. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **train_b**: Training example. Approach and repeatedly attempt to push an object with visible space beyond it; it stays fixed while the actor can retreat. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **control_a**: Training control. Apply similar attempts to a visually similar movable object; it displaces, countering an appearance-only immovability rule. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **validation_a**: Held-out validation example. Approach and repeatedly attempt to push an object with visible space beyond it; it stays fixed while the actor can retreat. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **test_a**: Frozen final example. Approach and repeatedly attempt to push an object with visible space beyond it; it stays fixed while the actor can retreat. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **test_control_a**: Frozen final control. Apply similar attempts to a visually similar movable object; it displaces, countering an appearance-only immovability rule. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
...
Main trials repeatedly fail to displace an object with visibly clear space beyond it; the actor can approach and retreat. Controls use a visually similar movable object. Failed observed displacement is evidence; a hidden mass or immovable label is not a recognizer fact.
...
|---|---|---|---|
| train_a | none | none | none |
| train_b | none | none | none |
| control_a | none | none | 5, 6, 8 |
| validation_a | none | none | none |
| test_a | none | none | none |
| test_control_a | none | none | 5, 6, 8 |
...
|---|---|---|
| `recordings/events_tests/push_immovable_train_a` | training | 11 |
| `recordings/events_tests/push_immovable_train_b` | training | 13 |
| `recordings/events_tests/push_immovable_control_a` | training_control | 15 |
| `recordings/events_tests/push_immovable_validation_a` | validation | 11 |
| `recordings/events_tests/push_immovable_test_a` | test | 13 |
| `recordings/events_tests/push_immovable_test_control_a` | test_control | 15 |
