...
Movement begins, continues, then stops. Includes a measured stationary warm-up interval.
...
- Frame 2: `{'predicate': 'start', 'args': [{'predicate': 'move', 'args': ['actor']}]}` (decision uses only frames 0 through 2).
- Frame 3: `{'predicate': 'continue', 'args': [{'predicate': 'move', 'args': ['actor']}]}` (decision uses only frames 0 through 3).
- Frame 4: `{'predicate': 'end', 'args': [{'predicate': 'move', 'args': ['actor']}]}` (decision uses only frames 0 through 4).
...
`recordings/events_tests/move_warmup_v2` contains 5 numbered frames from 0.
