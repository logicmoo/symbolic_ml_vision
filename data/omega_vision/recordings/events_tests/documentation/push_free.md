...
Actor approaches, contacts a crate, then advances into it across multiple steps
...
Contact plus action predicts crate movement in the same direction
...
Move without contact; move away; pause; vary crate/actor appearance and location.

- **train_a**: Training example. Approach the crate, push it into clear space, pause, push again and retreat; compare actual input attempts with displacement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **train_b**: Training example. Approach the crate, push it into clear space, pause, push again and retreat; compare actual input attempts with displacement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **control_a**: Training control. Issue directional movements in the adjacent lane without contacting the crate; only the actor moves. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **validation_a**: Held-out validation example. Approach the crate, push it into clear space, pause, push again and retreat; compare actual input attempts with displacement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **test_a**: Frozen final example. Approach the crate, push it into clear space, pause, push again and retreat; compare actual input attempts with displacement. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **test_control_a**: Frozen final control. Issue directional movements in the adjacent lane without contacting the crate; only the actor moves. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
...
Repeated contact-direction inputs displace a crate only into clear cells. Pause, retreat and no-contact controls separate actor commands from crate outcomes. Infer signed observed clearance, not clearance from a missing obstacle detection.
...
|---|---|---|
| `recordings/events_tests/push_free_train_a` | training | 11 |
| `recordings/events_tests/push_free_train_b` | training | 13 |
| `recordings/events_tests/push_free_control_a` | training_control | 14 |
| `recordings/events_tests/push_free_validation_a` | validation | 11 |
| `recordings/events_tests/push_free_test_a` | test | 13 |
| `recordings/events_tests/push_free_test_control_a` | test_control | 14 |
