...
A platform picks up, transports, then leaves its load.

**Execution status: not run.**

## Local memory required for this test

Distinct carrier/load IDs, supported load relation, relative pose and joint motion history

## Measured new evidence and decision / episode timing

Contact/support candidate at `1`; transported fixed-offset load at `2`; separation at `3`

## Required caution and fixture insufficiency

Without an earlier accepted carrier/load model, classify carry no earlier than `2`. Contact or co-motion alone is insufficient; do not assume force/support from fixture names.
...
|---|---|---|
| `recordings/events_tests/carry` | event_baseline | 4 |
