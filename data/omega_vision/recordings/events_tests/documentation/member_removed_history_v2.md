# member removed history v2

One member leaves while a two-member assembly survives. Includes independent observations before the membership change.
...
- Removal is not dissolution: two members still form the group.
...
- Frame 4: `{'predicate': 'member_removed', 'args': ['third', 'group']}` (decision uses only frames 0 through 4).
...
`recordings/events_tests/member_removed_history_v2` contains 6 numbered frames from 0.
