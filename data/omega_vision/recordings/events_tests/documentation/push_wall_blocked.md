...
Push crate against a wall; repeat attempt; remove obstacle and try again
...
Learn an observable blockage exception to the free-push rule
...
No force-through-wall; unknown destination remains ungraded; wall removal restores the positive case.

- **train_a**: Training example. Push up to the wall and repeat blocked attempts, click the actual barrier, then push through the now-clear destination. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **train_b**: Training example. Push up to the wall and repeat blocked attempts, click the actual barrier, then push through the now-clear destination. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **control_a**: Training control. Push up to the wall, then click elsewhere; the wall stays present and further attempts remain blocked. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=absent.
- **validation_a**: Held-out validation example. Push up to the wall and repeat blocked attempts, click the actual barrier, then push through the now-clear destination. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **test_a**: Frozen final example. Push up to the wall and repeat blocked attempts, click the actual barrier, then push through the now-clear destination. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=present.
- **test_control_a**: Frozen final control. Push up to the wall, then click elsewhere; the wall stays present and further attempts remain blocked. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=absent.
...
A visible wall stops the crate after an initial push. Repeated further attempts fail. A coordinate CLICK on that barrier removes it, and subsequent attempts succeed. Clicking elsewhere in controls does not remove the wall.
...
|---|---|---|---|
| train_a | none | none | 3, 10, 11 |
| train_b | none | none | 4, 12, 13 |
| control_a | none | none | 5 |
| validation_a | none | none | 3, 10, 11 |
| test_a | none | none | 4, 12, 13 |
| test_control_a | none | none | 5 |
...
|---|---|---|
| `recordings/events_tests/push_wall_blocked_train_a` | training | 16 |
| `recordings/events_tests/push_wall_blocked_train_b` | training | 19 |
| `recordings/events_tests/push_wall_blocked_control_a` | training_control | 22 |
| `recordings/events_tests/push_wall_blocked_validation_a` | validation | 16 |
| `recordings/events_tests/push_wall_blocked_test_a` | test | 19 |
| `recordings/events_tests/push_wall_blocked_test_control_a` | test_control | 22 |
