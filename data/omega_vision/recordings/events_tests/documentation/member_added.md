...
A third piece joins an existing two-member assembly.

**Execution status: not run.**

## Local memory required for this test

Established group's ID and membership set, plus the independent third track

## Measured new evidence and decision / episode timing

Compare before/after sets at `1`; joint motion at `2` corroborates the third member joining while group identity remains stable

## Required caution and fixture insufficiency

Requires a prior accepted group, not just two touching shapes. Group and member IDs remain distinct.
...
|---|---|---|
| `recordings/events_tests/member_added` | event_baseline | 3 |
