...
A previously visible interior object cannot be found.

**Execution status: not run.**

## Local memory required for this test

Frame `0`'s actor identity, last visible mask/position and complete current comparison

## Measured new evidence and decision / episode timing

At `1` fail to find the actor; retain its last-observed record through `2` rather than deleting the identity

## Required caution and fixture insufficiency

No supported exit or occluder explanation; missing is not destruction.
...
|---|---|---|
| `recordings/events_tests/missing` | event_baseline | 3 |
