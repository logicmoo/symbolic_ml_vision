...
In a separate mechanism, crate continues after release and later slows
...
Learn the need for velocity/history context, not the unconditional stop-on-release rule
...
Same visible initial geometry as release case; different observed dynamics; no oracle mechanism tag as input.

- **train_a**: Training example. Deliver a contact impulse, leave the lane and observe continued crate movement with increasing intervals before it stops. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **train_b**: Training example. Deliver a contact impulse, leave the lane and observe continued crate movement with increasing intervals before it stops. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **control_a**: Training control. Move without an initial contact impulse, then wait; the crate does not drift. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
- **validation_a**: Held-out validation example. Deliver a contact impulse, leave the lane and observe continued crate movement with increasing intervals before it stops. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **test_a**: Frozen final example. Deliver a contact impulse, leave the lane and observe continued crate movement with increasing intervals before it stops. Trace: plate occupancy=not applicable; crate movement=present; door changes=not applicable; wall removal=not applicable.
- **test_control_a**: Frozen final control. Move without an initial contact impulse, then wait; the crate does not drift. Trace: plate occupancy=not applicable; crate movement=absent; door changes=not applicable; wall removal=not applicable.
...
|---|---|---|---|
| train_a | none | none | 3, 4, 6, 9 |
| train_b | none | none | 4, 5, 6, 7 |
| control_a | none | none | none |
| validation_a | none | none | 3, 4, 5, 6 |
| test_a | none | none | 4, 5, 7, 10 |
...
|---|---|---|
| `recordings/events_tests/push_inertial_train_a` | training | 18 |
| `recordings/events_tests/push_inertial_train_b` | training | 19 |
| `recordings/events_tests/push_inertial_control_a` | training_control | 19 |
| `recordings/events_tests/push_inertial_validation_a` | validation | 18 |
| `recordings/events_tests/push_inertial_test_a` | test | 19 |
| `recordings/events_tests/push_inertial_test_control_a` | test_control | 19 |
