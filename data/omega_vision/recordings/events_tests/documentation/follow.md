...
Blue follows the red leader around a corner, then takes a different path.

**Execution status: not run.**

## Local memory required for this test

Leader/follower trajectory windows, temporal lag and prior episode

## Measured new evidence and decision / episode timing

At `2`, two lagged path matches support following; at `3` the follower diverges

## Required caution and fixture insufficiency

One coincident path point at oracle frame `1` is insufficient under the proposed two-match gate. Version the warm-up/timing; parallel motion alone is not following.
...
|---|---|---|
| `recordings/events_tests/follow` | event_baseline | 4 |
