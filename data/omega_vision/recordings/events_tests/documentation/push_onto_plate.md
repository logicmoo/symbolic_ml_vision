...
Push crate onto a plate, leave it there, then push it off
...
Compose independently supported push and plate-response predictions
...
Actor approaches without moving crate; crate misses plate; actor leaves while crate remains; preserve intermediate crate/plate/door evidence.

- **train_a**: Training example. Push onto the plate, leave the crate holding it while the actor retreats, then return and push it off; retain each intermediate visible response. Trace: plate occupancy=present; crate movement=present; door changes=present; wall removal=not applicable.
- **train_b**: Training example. Push onto the plate, leave the crate holding it while the actor retreats, then return and push it off; retain each intermediate visible response. Trace: plate occupancy=present; crate movement=present; door changes=present; wall removal=not applicable.
- **control_a**: Training control. Push through an adjacent lane; the crate moves but misses the plate and the door remains closed. Trace: plate occupancy=absent; crate movement=present; door changes=absent; wall removal=not applicable.
- **validation_a**: Held-out validation example. Push onto the plate, leave the crate holding it while the actor retreats, then return and push it off; retain each intermediate visible response. Trace: plate occupancy=present; crate movement=present; door changes=present; wall removal=not applicable.
- **test_a**: Frozen final example. Push onto the plate, leave the crate holding it while the actor retreats, then return and push it off; retain each intermediate visible response. Trace: plate occupancy=present; crate movement=present; door changes=present; wall removal=not applicable.
- **test_control_a**: Frozen final control. Push through an adjacent lane; the crate moves but misses the plate and the door remains closed. Trace: plate occupancy=absent; crate movement=present; door changes=absent; wall removal=not applicable.
...
|---|---|---|
| `recordings/events_tests/push_onto_plate_train_a` | training | 25 |
| `recordings/events_tests/push_onto_plate_train_b` | training | 30 |
| `recordings/events_tests/push_onto_plate_control_a` | training_control | 35 |
| `recordings/events_tests/push_onto_plate_validation_a` | validation | 25 |
| `recordings/events_tests/push_onto_plate_test_a` | test | 30 |
| `recordings/events_tests/push_onto_plate_test_control_a` | test_control | 35 |
